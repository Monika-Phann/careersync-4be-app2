...
exports.register = async (req, res) => {
  try {
    ...
    // 1. Register the user
    const newUser = await authService.registerUser(req.body, profileImageUrl);

    // 2. Normalize role for the check
    const normalizedRole = (req.body.role_name || req.body.role || "").toLowerCase().trim();

    // 3. Admin Notification Logic
    if (normalizedRole === 'mentor' || req.body.industry) {
      console.log("🚀 Role matched 'mentor'. Preparing admin email...");
      
      const emailService = require("../services/email.service");
      const adminEmail = process.env.ADMIN_EMAIL || 'careersync168@gmail.com';
      
      // Setup email content
      const subject = "🚨 Action Required: New Mentor Registration";
      const htmlContent = `
        <h3>New Mentor Approval Needed</h3>
        <p><strong>Mentor Email:</strong> ${req.body.email}</p>
        <p><strong>Status:</strong> Pending Approval</p>
      `;

      try {
        // ✅ CORRECTED: Passing arguments individually, NOT as one object
        // Arguments: (to, subject, html, text)
        await emailService.sendEmail(
          adminEmail, 
          subject, 
          htmlContent, 
          `New Mentor Registration: ${req.body.email}`
        );
        console.log(`✅ SUCCESS: Admin notification sent to ${adminEmail}`);
      } catch (mailErr) {
        // This will now catch actual SendGrid errors or "sendEmail is not a function" errors
        console.error("❌ MAIL ERROR:", mailErr.message);
      }
    } else {
      console.log("ℹ️ Skipping admin email: User is not a mentor. Role:", normalizedRole);
    }
